...
uintptr_t
ts_lua_escape_internal(u_char *dst, u_char *src, size_t size, int type)
{
    uint32_t       n;
    uint32_t       *escape;
    static u_char  hex[] = "0123456789abcdef";
    ...
    if (dst == NULL) {
        ...
        /* return the number of the characters to be escaped */
...
void
ts_lua_unescape_internal(u_char **dst, u_char **src, size_t size, int type)
{
    ...
    while (size--) {
        ...
        case sw_usual:
            if (ch == '?'
                && (type & (TS_UNESCAPE_URI|TS_UNESCAPE_REDIRECT)))
            {
                *d++ = ch;
                goto done;
            }
